...
def main():
    ...
    pdist_mat = loss.pairwise_distance(endpoints['emb'], squared=True)
    loss_mean, train_top1, prec_at_k, _, neg_dists, pos_dists = loss.LOSS_CHOICES[args.loss](
        pdist_mat, pids, args.margin, batch_precision_at_k=args.batch_k-1)
    ...
    tf.summary.histogram('embedding_dists', pdist_mat)
            ...
            for i in range(start_step, args.train_iterations):
                ...
                _, summary, step, b_prec_at_k, b_embs, b_loss, b_fids = \
                    sess.run([train_op, merged_summary, global_step,
                              prec_at_k, endpoints['emb'], loss_mean, fids])
                ...
                if i % 100 == 0:
                    log.info('iter:{:6d}, loss: {:.3f}, batch-p@{}: {:.2%}, ETA: {} ({:.2f}s/it)'.format(
                                step,
                                float(b_loss),
